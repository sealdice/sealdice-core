...
it('passes', async () => {
  ...
  assert(
    storyPainterOptionList.length >= 7,
    'story painter options should keep original option descriptions'
  );
  ...
  assert(
    storyPainterOptionList.some(
      item => item.key === 'commandHide' && item.desc.includes('正常显示指令结果')
    ),
