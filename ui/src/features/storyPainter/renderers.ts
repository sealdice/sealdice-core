...
import {
  escapeStoryPainterHtml,
  normalizeStoryPainterMessage,
  storyPainterNickname,
  storyPainterTime,
} from './formatters';
...
export function renderPreviewHtml(
  ...
  return [
    time
      ? `<span class="_time" style="color:${timeColor}">${escapeStoryPainterHtml(time)}</span>`
      : '',
...
export function renderForumText(
  ...
  const displayColor = forumOptions.bbsUseColorName
    ? colorHexToForumName(color)
    : color || fallbackColor;
  const time = storyPainterTime(item, options);
  ...
  const text = lines
    .map((line, index) => (index === 0 ? line : `[/color]\n${continuation}${line}`))
    .join('');
  return `${prefix}${text}[/color]`;
}
...
export function renderPineappleForumBlocks(
  ...
  items.forEach(item => {
    ...
    current = {
      ...
      color: forumOptions.bbsUseColorName
        ? colorHexToForumName(colorByItem(item))
        : colorByItem(item),
      lines: [text],
    };
  });
  ...
  return blocks.map(
    block =>
      `[color=silver]${block.name}[/color][color=${block.color}] ${block.lines.join('\n')} [/color]`
  );
}
...
export function renderTrgText(
  ...
  const text = message
    .split(/\r?\n/)
    .map(line => `${prefix}${speaker}${line}${voice}`)
    .join('\n');
...
function replaceCharacterBrackets(message: string, chars: StoryPainterChar[]): string {
  ...
  chars.forEach(char => {
    next = replaceAllText(
      next,
      `&lt;${escapeStoryPainterHtml(char.name)}&gt;`,
      escapeStoryPainterHtml(char.name)
    );
