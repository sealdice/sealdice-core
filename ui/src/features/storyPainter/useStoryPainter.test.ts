...
import type {
  StoryPainterParquetColumn,
  StoryPainterParquetDataset,
  StoryPainterParquetRow,
} from './parquetDataset';
...
it('passes', async () => {
  ...
  await painter.setDataset(
    createMemoryParquetDataset([
      {
        id: 1,
        nickname: 'Alice',
        IMUserId: '1001',
  ...
  assertDeepEqual(
    painter.items.value.map(item => item.message),
    ['', '']
  );
  ...
  await datasetPainter.setDataset(
    createMemoryParquetDataset([
      {
        id: 1,
        nickname: 'Alice',
  ...
  assertDeepEqual(
    rows.map(item => item.message),
    ['message-124', 'message-121', 'message-123']
  );
  assertDeepEqual(
    counted.calls
      .filter(call => call.columns.includes('message'))
