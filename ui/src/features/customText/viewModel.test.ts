...
it('passes', async () => {
  ...
  assertEqual(
    sortCustomTextCategory({ texts, helpInfo, category: 'core', filterMode: 'modified' })[0]![1]
      .length,
    1
  );
  assertEqual(
  ...
  const compact = buildCustomTextExportContent({
    texts,
    category: 'core',
    onlyCurrent: true,
    compact: true,
  ...
  const pretty = buildCustomTextExportContent({
    texts,
    category: 'core',
    onlyCurrent: false,
    compact: false,
