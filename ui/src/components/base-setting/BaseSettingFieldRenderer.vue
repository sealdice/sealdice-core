...
<template>
  <n-form-item :label="field.label" :path="field.key || field.id" class="base-setting-field" :data-field-id="field.id">
    ...
    <template v-if="field.key === 'QQEnablePoke'" #feedback>
      启用前请确认你使用的 QQ 连接方式支持该功能，若不支持请关闭该功能来避免日志中出现相关报错。
    </template>
    <template v-else-if="field.hint" #feedback>
      {{ field.hint }}
    </template>
  </n-form-item>
</template>
