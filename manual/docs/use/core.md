...
`.master quitgroup <群组 ID > <理由(可选)>`  从指定群组中退出，必须在同一平台使用。
...
`.ban add <统一ID> (<原因>)` 使用统一 ID，将指定用户拉入黑名单，理由可不填。

`.ban trust <统一ID>` 使用统一 ID，将指定用户添加至信任列表。

`.ban rm <统一ID>` 使用统一 ID，将指定用户移出黑名单/信任列表。

`.ban list` 展示列表。

`.ban list ban/warn/trust` 只显示被禁用/被警告/信任用户。

`.ban query <统一ID>` 查看指定用户黑白名单情况。
