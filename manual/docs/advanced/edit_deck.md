...
当然，如果你的海豹版本较新，我们也非常推荐你尝试 `toml` 格式的牌堆编写。`toml` 格式诞生较晚，支持更多现代特性，海豹为这种格式的牌堆支持了更多功能。

::: info 用什么打开&编写牌堆？

务必使用更专业的编辑器来编写牌堆，如果你不知道选择什么，可以尝试使用 [vscode](https://code.visualstudio.com) 或者 [sublime text](https://www.sublimetext.com)，这些都是被广泛使用的优秀编辑器。

更专业的编辑器能更好地帮助你编写牌堆。这些编辑器提供的语法高亮、格式化、错误检查、编码转换等功能，能带来更好的牌堆编写体验（可能需要安装插件）。
