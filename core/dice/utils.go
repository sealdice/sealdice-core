...
// GetCtxStandInFirst 等着后续再加 @名字 版本，以及 @team 版本 // pos int
func GetCtxStandInFirst(ctx *MsgContext, cmdArgs *CmdArgs, setTempVar bool) (*MsgContext, bool) {
	for _, i := range cmdArgs.At {
		if i.UID == ctx.conn.UniformID {
			continue
		}
