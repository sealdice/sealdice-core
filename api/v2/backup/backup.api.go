...
func (s *BackupService) GetList(_ context.Context, _ *request.Empty) (*response.ItemResponse[FileListResp], error) {
	...
	return response.NewItemResponse[FileListResp](FileListResp{Items: items}), nil
}

func (s *BackupService) Download(_ context.Context, ol *DownloadReq) (*huma.StreamResponse, error) {
	if s.dm.JustForTest {
...
func (s *BackupService) Delete(_ context.Context, ol *DeleteReq) (*response.ItemResponse[response.SimpleOK], error) {
	name := ol.Body.Name
	var err error
	if name != "" && (!strings.Contains(name, "/")) && (!strings.Contains(name, "\\")) {
		err = os.Remove(filepath.Join(dice.BackupDir, name))
	}
	return response.NewItemResponse[response.SimpleOK](response.SimpleOK{Success: err == nil}), nil
}

func (s *BackupService) BatchDelete(_ context.Context, ol *BatchDeleteReq) (*response.ItemResponse[response.BatchDeleteResp], error) {
	v := ol.Body
...
func (s *BackupService) Exec(_ context.Context, ol *ExecRunReq) (*response.ItemResponse[response.SimpleOK], error) {
	_, err := s.dm.Backup(dice.BackupSelection(ol.Body.Selection), false)
	return response.NewItemResponse[response.SimpleOK](response.SimpleOK{Success: err == nil}), nil
}

func (s *BackupService) ConfigGet(_ context.Context, _ *request.Empty) (*response.ItemResponse[Config], error) {
	bc := Config{}
	...
	return response.NewItemResponse[Config](bc), nil
}

func (s *BackupService) ConfigSave(_ context.Context, ol *ConfigSaveReq) (*response.ItemResponse[struct{}], error) {
	v := ol.Body
