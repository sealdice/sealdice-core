...
func (s *Service) GetText(_ context.Context, _ *request.Empty) (*response.ItemResponse[customtextm.TextResp], error) {
	return response.NewItemResponse[customtextm.TextResp](customtextm.TextResp{
...
func (s *Service) SaveCategory(_ context.Context, req *customtextm.SaveCategoryReq) (*response.ItemResponse[cmm.SimpleOK], error) {
	category := strings.TrimSpace(req.Category)
	if category == "" {
		return nil, huma.Error400BadRequest("missing category")
	}
	data := normalizeTextTemplate(req.Body.Body.Data)
	...
	return response.NewItemResponse[cmm.SimpleOK](cmm.SimpleOK{Success: true}), nil
}

func (s *Service) PreviewRefresh(_ context.Context, req *customtextm.PreviewRefreshReq) (*response.ItemResponse[cmm.SimpleOK], error) {
	category := strings.TrimSpace(req.Category)
	if category == "" {
		return nil, huma.Error400BadRequest("missing category")
	}
