...
func TestGetConfigReturnsCurrentReplyConfig(t *testing.T) {
	...
	resp, err := svc.GetConfig(t.Context(), &FilenamePath{Filename: "reply.yaml"})
	if err != nil {
...
`)
	...
	resp, err := svc.GetRules(t.Context(), &RulePageQuery{
		Filename: "reply.yaml",
		Page:     2,
		PageSize: 1,
	})
...
`)
	...
	resp, err := svc.GetConditions(t.Context(), &ConditionPageQuery{
		Filename: "reply.yaml",
		Page:     2,
		PageSize: 1,
	})
...
func TestCreateFileRejectsExistingName(t *testing.T) {
	...
	resp, err := svc.CreateFile(t.Context(), &FileReq{
		Body: FileBody{Filename: "reply.yaml"},
	})
	if err != nil {
...
func TestCreateAndDeleteFile(t *testing.T) {
	...
	createResp, err := svc.CreateFile(t.Context(), &FileReq{
		Body: FileBody{Filename: "reply2.yaml"},
	})
	if err != nil {
		t.Fatalf("CreateFile returned error: %v", err)
	}
	...
	deleteResp, err := svc.DeleteFile(t.Context(), &FilenamePath{Filename: "reply2.yaml"})
	if err != nil {
...
func TestDownloadRejectsInvalidName(t *testing.T) {
	svc := newTestService(t)
	if _, err := svc.Download(t.Context(), &FilenamePath{Filename: "../bad.yaml"}); err == nil {
...
func TestDownloadReturnsStreamForExistingFile(t *testing.T) {
	...
	resp, err := svc.Download(t.Context(), &FilenamePath{Filename: "reply.yaml"})
	if err != nil {
...
func TestDebugModeRoundTrip(t *testing.T) {
	...
	setResp, err := svc.SetDebugMode(t.Context(), &DebugModeReq{
		Body: DebugModeResp{Value: true},
	})
	if err != nil {
...
func TestUploadFileSavesMultipartPayload(t *testing.T) {
	...
	_, err = svc.Upload(t.Context(), &UploadReq{RawBody: raw})
	if err != nil {
		t.Fatalf("Upload returned error: %v", err)
	}

	itemsResp, err := svc.GetFileList(t.Context(), &FileListQuery{})
	if err != nil {
		t.Fatalf("GetFileList returned error: %v", err)
	}
	found := false
