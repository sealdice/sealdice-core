...
import (
	...
	basesettingm "sealdice-core/api/v2/model/basesetting"
	"sealdice-core/dice"
...
func (s *Service) GetSchema(_ context.Context, _ *request.Empty) (*response.ItemResponse[basesettingm.BaseSettingSchemaResp], error) {
	return response.NewItemResponse(buildBaseSettingSchema()), nil
}

func (s *Service) GetValue(_ context.Context, _ *request.Empty) (*response.ItemResponse[basesettingm.BaseSettingValueResp], error) {
	return response.NewItemResponse(s.buildValue()), nil
}

func (s *Service) SetValue(_ context.Context, req *basesettingm.BaseSettingUpdateReq) (*response.ItemResponse[basesettingm.BaseSettingActionResp], error) {
	if err := s.applyPatch(req.Body.Body); err != nil {
		return nil, err
	}
	s.dice.MarkModified()
	s.dice.Parent.Save()
	return response.NewItemResponse(basesettingm.BaseSettingActionResp{Success: true}), nil
}

func (s *Service) MailTest(_ context.Context, _ *request.Empty) (*response.ItemResponse[basesettingm.BaseSettingActionResp], error) {
...
func (s *Service) Upgrade(_ context.Context, req *basesettingm.BaseSettingUpgradeReq) (*response.ItemResponse[basesettingm.BaseSettingActionResp], error) {
...
func (s *Service) buildValue() basesettingm.BaseSettingValueResp {
	password := ""
	...
	extDefaultSettings := make([]*basesettingm.BaseSettingExtDefaultSettingItem, 0, len(s.dice.Config.ExtDefaultSettings))
	for _, item := range s.dice.Config.ExtDefaultSettings {
		if item == nil || !item.Loaded {
			continue
		}
		extDefaultSettings = append(extDefaultSettings, &basesettingm.BaseSettingExtDefaultSettingItem{
			Name:            item.Name,
	...
	return basesettingm.BaseSettingValueResp{
...
func buildBaseSettingSchema() basesettingm.BaseSettingSchemaResp {
	option := func(label, value string) *basesettingm.BaseSettingOption {
		return &basesettingm.BaseSettingOption{Label: label, Value: value}
	}
	field := func(id, key, label, kind string, keywords ...string) *basesettingm.BaseSettingFieldSchema {
		return &basesettingm.BaseSettingFieldSchema{
			ID:       id,
			Key:      key,
			Label:    label,
			Kind:     kind,
	...
	return basesettingm.BaseSettingSchemaResp{
		Tabs: []*basesettingm.BaseSettingTabSchema{
			{
				ID:    "master-notice",
				Title: "Master 与通知",
				Groups: []*basesettingm.BaseSettingGroupSchema{
					{
						ID:    "master",
						Title: "Master 管理",
						Fields: []*basesettingm.BaseSettingFieldSchema{
				...
				Groups: []*basesettingm.BaseSettingGroupSchema{
					{
						ID:    "rate-limit-main",
						Title: "刷屏警告设置",
						Notes: []*basesettingm.BaseSettingNote{
				...
				Groups: []*basesettingm.BaseSettingGroupSchema{
					{
						...
						Title: "UI 访问控制",
						Fields: []*basesettingm.BaseSettingFieldSchema{
							{ID: "serve-address", Key: "serveAddress", Label: "UI 界面地址", Kind: "select", AllowCustomValue: true, Options: []*basesettingm.BaseSettingOption{option("0.0.0.0:3211", "0.0.0.0:3211"), option("127.0.0.1:3211", "127.0.0.1:3211")}, Keywords: []string{"服务地址", "UI地址", "端口", "公网"}},
...
func extractUpgradeForm(raw huma.MultipartFormFiles[basesettingm.BaseSettingUpgradeForm]) (*basesettingm.BaseSettingUpgradeForm, error) {
	data := raw.Data()
	...
	return &basesettingm.BaseSettingUpgradeForm{
		File: huma.FormFile{
			File:        file,
