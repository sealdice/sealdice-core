...
func (s *Service) GetStatus(_ context.Context, _ *cmn.Empty) (*jsm.StatusItemResponse, error) {
	return response.NewItemResponse(jsm.JsStatusResp{
		Status: s.dice.Config.JsEnable,
	}), nil
}

func (s *Service) GetList(_ context.Context, req *jsm.ListQuery) (*jsm.ListItemResponse, error) {
	if !s.dice.Config.JsEnable {
		return response.NewItemResponse(jsm.JsListResp{Success: true, Data: []jsm.JsInfo{}, Total: 0}), nil
	}

	items := make([]jsm.JsInfo, 0, len(s.dice.JsScriptList))
	...
	var paged []jsm.JsInfo
	if err := pager.Get(&paged); err != nil {
		return nil, huma.Error500InternalServerError("分页失败")
	}

	return response.NewItemResponse(jsm.JsListResp{
		Success:  true,
		Data:     paged,
		Total:    int(total),
...
func (s *Service) GetRecord(_ context.Context, _ *cmn.Empty) (*jsm.ExecuteItemResponse, error) {
	if s.dice.JsPrinter == nil {
		return response.NewItemResponse(jsm.JsExecuteResp{Outputs: []string{}}), nil
	}
	outputs := s.dice.JsPrinter.RecordEnd()
	return response.NewItemResponse(jsm.JsExecuteResp{Outputs: outputs}), nil
}

func (s *Service) GetConfigs(_ context.Context, req *struct {
	Name string `query:"name"`
}) (*jsm.ConfigMapItemResponse, error) {
	cm := s.dice.ConfigManager
	if cm == nil {
		return response.NewItemResponse(jsm.PluginConfigMap{}), nil
	}

	result := make(jsm.PluginConfigMap)
	for name, pc := range cm.Plugins {
		...
		result[name] = &jsm.APIPluginConfig{
...
func (s *Service) GetDeadConfigs(_ context.Context, _ *cmn.Empty) (*jsm.DeadConfigsItemResponse, error) {
	cm := s.dice.ConfigManager
	if cm == nil {
		return response.NewItemResponse(jsm.DeadConfigsResp{Configs: []jsm.DeadConfig{}}), nil
	}

	var dead []jsm.DeadConfig
	for name := range cm.Plugins {
		if s.findPluginByName(name) != nil {
			continue
		}
		dead = append(dead, jsm.DeadConfig{Name: name})
	}
	sort.Slice(dead, func(i, j int) bool { return dead[i].Name < dead[j].Name })

	return response.NewItemResponse(jsm.DeadConfigsResp{Configs: dead}), nil
}

func (s *Service) DataList(_ context.Context, req *struct {
	Name string `path:"name"`
	jsm.DataListQuery
}) (*jsm.DataListItemResponse, error) {
...
}) (*jsm.DataValueItemResponse, error) {
	if req.Key == "" {
...
func (s *Service) DataInfo(_ context.Context, req *jsm.NamePath) (*jsm.DataInfoItemResponse, error) {
	st, err := resolveStorage(s.dice, req.Name)
	if err != nil {
		return nil, huma.Error404NotFound(err.Error())
	}
...
func (s *Service) Reload(_ context.Context, _ *cmn.Empty) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
	...
	s.dice.JsReload()
	return response.NewItemResponse(jsm.JsSimpleResult{Success: true}), nil
}

func (s *Service) Shutdown(_ context.Context, _ *cmn.Empty) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
	s.dice.JsShutdown()
...
func (s *Service) Enable(_ context.Context, req *jsm.NameReq) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
	...
	dice.JsEnable(s.dice, req.Body.Body.Name)
	return response.NewItemResponse(jsm.JsSimpleResult{Success: true}), nil
}

func (s *Service) Disable(_ context.Context, req *jsm.NameReq) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
	if req.Body.Body.Name == "" {
	...
	dice.JsDisable(s.dice, req.Body.Body.Name)
	return response.NewItemResponse(jsm.JsSimpleResult{Success: true}), nil
}

func (s *Service) Delete(_ context.Context, req *jsm.FilenameReq) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
	if !s.dice.Config.JsEnable {
...
func (s *Service) Execute(_ context.Context, req *jsm.ExecuteReq) (*jsm.ExecuteItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsExecuteResp{Outputs: []string{}}), nil
	}
	if !s.dice.Config.JsEnable {
	...
	loop.RunOnLoop(func(vm *goja.Runtime) {
		...
		code := fmt.Sprintf("(function(exports, require, module) { %s })();", req.Body.Body.Value)
...
func (s *Service) CheckUpdate(_ context.Context, req *jsm.CheckUpdateReq) (*jsm.CheckUpdateItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsCheckUpdateResp{Success: false, Err: "展示模式不支持该操作"}), nil
	}
	if req.Body.Body.Filename == "" {
		return nil, huma.Error400BadRequest("filename不能为空")
	}
...
func (s *Service) Update(_ context.Context, req *jsm.UpdateReq) (*jsm.UpdateItemResponse, error) {
	if s.dm.JustForTest {
...
func (s *Service) Upload(_ context.Context, req *jsm.UploadReq) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
...
func (s *Service) InitUpload(_ context.Context, req *jsm.UploadInitReq) (*response.ItemResponse[jsm.JsUploadSessionResp], error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsUploadSessionResp{
			Success:         false,
...
func (s *Service) GetUploadChunkStatus(_ context.Context, req *jsm.UploadChunkQuery) (*response.ItemResponse[jsm.JsUploadChunkResp], error) {
	session, err := s.uploadManager.Get(req.SessionID)
	if err != nil {
		return nil, huma.Error404NotFound("上传会话不存在")
	}
	return response.NewItemResponse(jsm.JsUploadChunkResp{
...
func (s *Service) UploadChunk(_ context.Context, req *jsm.UploadChunkReq) (*response.ItemResponse[jsm.JsUploadChunkResp], error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsUploadChunkResp{
			Success:       false,
			UploadedChunk: req.Index,
...
func (s *Service) CompleteUpload(_ context.Context, req *jsm.UploadCompleteReq) (*response.ItemResponse[jsm.JsUploadCompleteResp], error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsUploadCompleteResp{
			Success:  false,
			TestMode: true,
		}), nil
	}
...
func (s *Service) SetConfigs(_ context.Context, req *jsm.SetConfigsReq) (*jsm.SimpleItemResponse, error) {
	...
	cm := s.dice.ConfigManager
	var errs []error
	for key, val := range req.Body.Body.Config {
		if err := cm.SetConfig(req.Body.Body.Name, key, val); err != nil {
...
func (s *Service) ResetConfig(_ context.Context, req *jsm.ResetConfigReq) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
	if !s.dice.Config.JsEnable {
	...
	cm := s.dice.ConfigManager
	for _, key := range req.Body.Body.Keys {
		cm.ResetConfigToDefault(req.Body.Body.Name, key)
	}
	return response.NewItemResponse(jsm.JsSimpleResult{Success: true}), nil
}

func (s *Service) DeleteDeadConfigs(_ context.Context, req *jsm.DeleteDeadConfigsReq) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
	cm := s.dice.ConfigManager
	for _, name := range req.Body.Body.Names {
		if s.findPluginByName(name) == nil {
...
func (s *Service) DataSet(_ context.Context, req *struct {
	...
	if err := st.setValue(req.Body.Body.Key, req.Body.Body.Value); err != nil {
		return nil, huma.Error500InternalServerError(err.Error())
	}
	return response.NewItemResponse(jsm.JsSimpleResult{Success: true}), nil
}
...
func (s *Service) DataDelete(_ context.Context, req *struct {
	...
	if err := st.deleteKeys(req.Body.Body.Keys); err != nil {
		return nil, huma.Error500InternalServerError(err.Error())
	}
	return response.NewItemResponse(jsm.JsSimpleResult{Success: true}), nil
}

func (s *Service) DataShrink(_ context.Context, req *jsm.NamePath) (*jsm.SimpleItemResponse, error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(jsm.JsSimpleResult{Success: false, TestMode: true}), nil
	}
	st, err := resolveStorage(s.dice, req.Name)
...
func sortJS(items []jsm.JsInfo, sortBy, sortOrder string) {
	desc := sortOrder == "desc"
	switch sortBy {
	case "author":
...
func extractUploadForm(raw huma.MultipartFormFiles[jsm.UploadForm]) *jsm.UploadForm {
	data := raw.Data()
