...
func (s *Service) GetStatus(_ context.Context, _ *request.Empty) (*response.ItemResponse[CensorStatusResp], error) {
	return response.NewItemResponse(s.buildStatusResp(false)), nil
}

func (s *Service) Restart(_ context.Context, _ *request.Empty) (*response.ItemResponse[CensorStatusResp], error) {
	if s.dm.JustForTest {
...
func (s *Service) Stop(_ context.Context, _ *request.Empty) (*response.ItemResponse[CensorStatusResp], error) {
	if s.dm.JustForTest {
		return response.NewItemResponse(s.buildStatusResp(true)), nil
	}
...
func (s *Service) GetConfig(_ context.Context, _ *request.Empty) (*response.ItemResponse[CensorConfigResp], error) {
	config := s.dice.Config
	return response.NewItemResponse(CensorConfigResp{
...
func (s *Service) SetConfig(_ context.Context, req *CensorConfigReq) (*response.ItemResponse[CensorConfigResp], error) {
...
func (s *Service) GetWords(_ context.Context, _ *request.Empty) (*response.ItemResponse[CensorWordsResp], error) {
	if err := s.checkReady(); err != nil {
		return nil, err
	}
	temp := map[string]*CensorWordItem{}
	for word, info := range s.dice.CensorManager.Censor.SensitiveKeys {
		switch info.Reason {
		case censorcore.Origin:
			if _, ok := temp[word]; !ok {
				temp[word] = &CensorWordItem{
					Main:  word,
		...
		case censorcore.IgnoreCase, censorcore.PinYin:
			...
			if !ok {
				temp[info.Origin] = &CensorWordItem{
					Main:  info.Origin,
					Level: int(info.Level),
				}
				sensitiveWord = temp[info.Origin]
			}
			sensitiveWord.Related = append(sensitiveWord.Related, CensorRelatedWord{
				Word:   word,
				Reason: int(info.Reason),
			})
		}
	}
	data := make([]*CensorWordItem, 0, len(temp))
	...
	return response.NewItemResponse(CensorWordsResp{Data: data}), nil
}

func (s *Service) GetFiles(_ context.Context, _ *request.Empty) (*response.ItemResponse[CensorFilesResp], error) {
	if err := s.checkReady(); err != nil {
		return nil, err
	}
	files := make([]*CensorFileInfo, 0, len(s.dice.CensorManager.SensitiveWordsFiles))
	for _, f := range s.dice.CensorManager.SensitiveWordsFiles {
		files = append(files, &CensorFileInfo{
			Key:      f.Key,
			Count:    f.FileCounter,
			FileType: f.FileType,
			Name:     f.Name,
	...
	return response.NewItemResponse(CensorFilesResp{Data: files}), nil
}

func (s *Service) UploadFile(_ context.Context, req *CensorUploadReq) (*response.ItemResponse[CensorSimpleResp], error) {
	if err := s.checkReady(); err != nil {
		return nil, err
	}
...
func (s *Service) GetLogsPage(_ context.Context, req *CensorLogPageQuery) (*response.ItemResponse[CensorLogPageResp], error) {
	if err := s.checkReady(); err != nil {
		return nil, err
	}
	pageNum := req.PageNum
	...
	return response.NewItemResponse(CensorLogPageResp{
		Data:     page,
		Total:    total,
		PageNum:  pageNum,
		PageSize: len(page),
...
func (s *Service) buildStatusResp(testMode bool) CensorStatusResp {
	isLoading := false
	if s.dice.CensorManager != nil {
		isLoading = s.dice.CensorManager.IsLoading
	}
	return CensorStatusResp{
...
func applyLevelConfig(config *dice.Config, level censorcore.Level, input CensorLevelConfig) {
...
func extractUploadForm(raw huma.MultipartFormFiles[CensorUploadForm]) (*CensorUploadForm, error) {
	data := raw.Data()
