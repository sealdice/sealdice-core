...
func TestEnableWorkflowAndQRCode(t *testing.T) {
	...
	if _, disableErr := svc.SetEnable(t.Context(), &EnableReq{
		ID:   ep.ID,
		Body: EnableBody{Enable: false},
	}); disableErr != nil {
...
func TestEditableConfigMasksSensitiveFieldsAndIncludesPlaceholders(t *testing.T) {
	...
	resp, err := svc.GetEditableConfig(t.Context(), &IDPath{ID: createResp.Body.Item.ID})
	if err != nil {
