...
func (s *Service) GetInfo(_ context.Context, _ *request.Empty) (*response.ItemResponse[StoryInfoResp], error) {
	info, err := service.LogGetInfo(s.dice.DBOperator)
	if err != nil {
		return nil, huma.Error500InternalServerError(err.Error())
	}
	return response.NewItemResponse(StoryInfoResp{
		TotalLogs:    info[0],
		TotalItems:   info[1],
		CurrentLogs:  info[2],
		CurrentItems: info[3],
...
func (s *Service) GetLogPage(_ context.Context, req *LogPageQuery) (*response.ItemResponse[LogPageResp], error) {
	...
	items := make([]StoryLogView, 0, len(page))
	for _, item := range page {
		if item == nil {
			continue
		}
		items = append(items, StoryLogView{
			ID:         item.ID,
			Name:       item.Name,
	...
	return response.NewItemResponse(LogPageResp{
		Result:   true,
		Total:    total,
		Data:     items,
...
func (s *Service) GetItemPage(_ context.Context, req *ItemPageQuery) (*response.ItemResponse[LogLinePageResp], error) {
...
func (s *Service) DeleteLog(_ context.Context, req *DeleteLogReq) (*response.ItemResponse[DeleteLogResp], error) {
	...
	err := service.LogDeleteByID(s.dice.DBOperator, req.Body.ID)
	if err != nil {
		if errors.Is(err, service.ErrLogNotFound) {
			return response.NewItemResponse(DeleteLogResp{Success: false}), nil
		}
		return nil, huma.Error500InternalServerError(err.Error())
	}
	return response.NewItemResponse(DeleteLogResp{Success: true}), nil
}

func (s *Service) UploadLog(_ context.Context, req *UploadLogReq) (*response.ItemResponse[UploadLogResp], error) {
	if req.Body.ID == 0 {
		return nil, huma.Error400BadRequest("缺少 id")
	}
	logInfo, err := service.LogGetByID(s.dice.DBOperator, req.Body.ID)
	if err != nil {
		if errors.Is(err, service.ErrLogNotFound) {
			return nil, huma.Error404NotFound("未找到该条日志")
		}
	...
	url0, uploadTime, updateTime, err := service.LogGetUploadInfoByID(s.dice.DBOperator, req.Body.ID)
	if err != nil {
		return nil, huma.Error500InternalServerError(err.Error())
	}
	if !req.Body.Force && url0 != "" && uploadTime > updateTime {
		return response.NewItemResponse(UploadLogResp{
			URL:    url0,
			Reused: true,
		}), nil
	}
	unofficial, url, err := logSendToBackend(s.dice, logInfo, req.Body.Force)
	if err != nil {
		return nil, huma.Error500InternalServerError(err.Error())
	}
	return response.NewItemResponse(UploadLogResp{
...
func (s *Service) GetBackupList(_ context.Context, _ *request.Empty) (*response.ItemResponse[BackupListResp], error) {
	list, err := dice.StoryLogBackupList(s.dice)
	if err != nil {
		return nil, huma.Error500InternalServerError(err.Error())
	}
	return response.NewItemResponse(BackupListResp{
		Result: true,
		Data:   list,
	}), nil
}

func (s *Service) DownloadBackup(ctx context.Context, req *BackupDownloadQuery) (*huma.StreamResponse, error) {
...
func (s *Service) BatchDeleteBackup(_ context.Context, req *BackupBatchDeleteReq) (*response.ItemResponse[BackupBatchDeleteResp], error) {
	fails := dice.StoryLogBackupBatchDelete(s.dice, req.Body.Names)
	return response.NewItemResponse(BackupBatchDeleteResp{
		Result: len(fails) == 0,
		Fails:  fails,
	}), nil
}

func (s *Service) PreviewCleanup(_ context.Context, req *CleanupPreviewQuery) (*response.ItemResponse[CleanupPreviewResp], error) {
	if req.Months < 0 {
	...
	return response.NewItemResponse(CleanupPreviewResp{
		Logs:          preview.Logs,
		Items:         preview.Items,
...
func (s *Service) Cleanup(_ context.Context, req *CleanupReq) (*response.ItemResponse[CleanupResp], error) {
	if req.Body.Months < 0 {
		return nil, huma.Error400BadRequest("months 不能小于 0")
	}
	result, err := service.LogCleanupByMonths(s.dice.DBOperator, req.Body.Months, req.Body.Vacuum)
	if err != nil {
		return nil, huma.Error500InternalServerError(err.Error())
	}
	return response.NewItemResponse(CleanupResp{
		Logs:     result.Logs,
		Items:    result.Items,
