...
type PlatformOnebot12 struct {
	...
	echoMap        *SyncMap[any, chan *MessageQQ2] `yaml:"-"`
...
type MessageQQ2 struct {
	MessageQQBase2
	Message string `json:"message"` // 消息内容
}

type MessageQQArray2 struct {
	MessageQQBase2
	Message []*OneBotV11MsgItem `json:"message"` // 消息内容
}

func (msgQQ *MessageQQ2) toStdMessage() *Message {
	msg := new(Message)
	msg.Time = msg.Time
...
func FormatDiceIDQQ2(diceQQ string) string {
	return fmt.Sprintf("QQ:%s", diceQQ)
}

func FormatDiceIDQQGroup2(diceQQ string) string {
	return fmt.Sprintf("QQ-Group:%s", diceQQ)
}

func FormatDiceIDQQCh2(userID string) string {
	return fmt.Sprintf("QQ-CH:%s", userID)
}

func FormatDiceIDQQChGroup2(guildID, channelID string) string {
	return fmt.Sprintf("QQ-CH-Group:%s-%s", guildID, channelID)
}

func ServeWechat(dice *Dice, conn *EndPointInfo, loginInfo GoCqhttpLoginInfo) {
	pa := conn.Adapter.(*PlatformOnebot12)
...
func tryParseOneBot11ArrayMessage2(log *zap.SugaredLogger, message string, writeTo *MessageQQ2) error {
	msgQQType2 := new(MessageQQArray2)
	err := json.Unmarshal([]byte(message), msgQQType2)

	if err != nil {
		log.Warn("无法解析 onebot12 字段:", message)
		return err
	}
	...
	writeTo.MessageQQBase2 = msgQQType2.MessageQQBase2
	writeTo.Message = cqMessage.String()
	return nil
}

func OneBot11CqMessageToArrayMessage2(longText string) []interface{} {
...
func (pa *PlatformOnebot12) Serve() int {
	pa.Implementation = "wechat"
	ep := pa.EndPoint
	s := pa.Session
	log := s.Parent.Logger
	dm := s.Parent.Parent
	...
	socket.OnConnected = func(socket gowebsocket.Socket) {
		ep.State = 1
		if pa.IsReverse {
			log.Info("onebot v12 反向ws连接成功")
		} else {
			log.Info("onebot v12 连接成功")
		}
	...
	socket.OnConnectError = func(err error, socket gowebsocket.Socket) {
		...
		// refused 不算大事
		log.Error("onebot v12 connection error: ", err)
		// }
	...
	socket.OnTextMessage = func(message string, socket gowebsocket.Socket) {
		...
		msgQQ := new(MessageQQ2)
		err := json.Unmarshal([]byte(message), msgQQ)

		if err != nil {
			err = tryParseOneBot11ArrayMessage2(log, message, msgQQ)
...
func (pa *PlatformOnebot12) DoRelogin() bool {
...
func (pa *PlatformOnebot12) SetEnable(enable bool) {
	d := pa.Session.Parent
	c := pa.EndPoint
	if enable {
		c.Enable = true
...
func (pa *PlatformOnebot12) SetQQProtocol2(protocol int) bool {
...
func (pa *PlatformOnebot12) IsInLogin2() bool {
	return pa.GoCqhttpState < StateCodeLoginSuccessed
}

func (pa *PlatformOnebot12) IsLoginSuccessed2() bool {
	return pa.GoCqhttpState == StateCodeLoginSuccessed
}

func (pa *PlatformOnebot12) packTempCtx2(msgQQ *MessageQQ2, msg *Message) *MsgContext {
	ep := pa.EndPoint
	session := pa.Session
...
func (pa *PlatformOnebot12) QuitGroup(_ *MsgContext, _ string) {}

func (pa *PlatformOnebot12) SetGroupCardName(_ *MsgContext, _ string) {}

func (pa *PlatformOnebot12) MemberBan(_ string, _ string, _ int64) {}

func (pa *PlatformOnebot12) MemberKick(_ string, _ string) {}

func (pa *PlatformOnebot12) EditMessage(_ *MsgContext, _, _ string) {}

func (pa *PlatformOnebot12) RecallMessage(_ *MsgContext, _ string) {}

func NewWechatConnectInfoItem(account string) *EndPointInfo {
	conn := new(EndPointInfo)
	conn.ID = uuid.New().String()
	conn.Platform = "QQ"
	conn.ProtocolType = "wechat"
	conn.Enable = false
	conn.RelWorkDir = "extra/go-cqhttp-qq" + account

	conn.Adapter = &PlatformOnebot12{
		EndPoint:          conn,
